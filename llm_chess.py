...
use_san_pgn = True  # Whether to use SAN (Standard Algebraic Notation) / PGN (Portable Game Notation) format for moves, both when return game state or moves
...
throttle_delay = 0.25  # some LLM providers might thorttle frequent API reuqests, make a delay (in seconds) between moves
...
# May help isolating thinking tokens.
...
def run(log_dir="_logs", save_logs=True):
    ...
    def get_current_board(force_unicode = False) -> (
        Annotated[str, "A text representation of the current board state"]
    ):
        if use_san_pgn and not force_unicode:
            # Build up the PGN string for the game so far, using the san_moves list
            # First, the PGN header
            pgn_header = (
                "[Event \"Chess\"]\n"
                "[Site \"Chess\"]\n"
                "[Date \"2025-02-06\"]\n"
                ...
                "[White \"Player_White\"]\n"
                "[Black \"Player_Black\"]\n"
                # The result should suggest that black won, since the LLM is playing black
                "[Result \"*\"]\n"
            )
            pgn = "Current state of the board:\n" + board.unicode() + "\n" + pgn_header
            # The san moves are stored in the san_moves list, with index 0 being the first white move, 1 being the first black move, etc.
            for i in range(len(san_moves)):
                if i % 2 == 0:
                    turn = (i // 2) + 1
                    pgn += f"{turn}. {san_moves[i]} "
                else:
                    pgn += f"{san_moves[i]}\n"
            return pgn
    ...
    def make_move(
        ...
        if use_san_pgn:
            # Double check that the move is one of the listed legal moves:
            legal_moves = get_legal_moves()
            unicode_board_state = get_current_board(force_unicode=True)
            legal_moves_list = legal_moves.split(",")
            if move not in legal_moves_list:
                raise ValueError(
                    f"Invalid move {move} provided."
                )
            # Check the validity of the move:
            uci_move = board.parse_san(str(move)).uci()
            # Get the san_moves list
            nonlocal san_moves
            # The move given by the player should already be in SAN format and valid
            san_moves.append(str(move))
            # Convert it back to UCI for the rest of the logic to work
            move = uci_move
    ...
    format_str = "SAN" if use_san_pgn else "UCI"
    format_board = "the PGN format of the game so far" if use_san_pgn else "the schema and current status of the board"
    format_example = "Bxe7" if use_san_pgn else "e2e4"

    common_prompt = (
        "Now is your turn to make a move. Before making a move please view the game so far:\n"
        f"- '{get_current_board_action}' to get {format_board}\n"
        # f"- '{get_legal_moves_action}' to get a {format_str} formatted list of available moves\n"
        + (
            f"- '{reflect_action}' to take a moment to think about your strategy\n"
            if enable_reflection
            else ""
        )
        + f"- '{make_move_action} <{format_str} formatted move>' when you are ready to complete your turn (e.g., '{make_move_action} {format_example}')"
    )
    ...
    reflection_followup_prompt = (
        ...
        f"{make_move_action} <{format_str} formatted move>"
    )
