...
def get_llms_autogen(temperature=None, reasoning_effort=None):
    ...
    """
    model_kinds = [
        os.environ.get("MODEL_KIND_W", "gemini"),
        os.environ.get("MODEL_KIND_B", "gemini"),
    ]
...
def save_video(filename):
    ...
    if _frames:
        ...
        clip.write_videofile(filename.replace(':', '.'), codec="libx264")
    else:
...
def display_store_game_video_and_stats(game_stats, log_dir="_logs", save_logs=True):
    ...
    if save_logs:
        log_filename = f"{log_dir}/{game_stats['time_started'].replace(':', '.')}.json"
        if os.path.exists(
            log_filename
        ):  # if running automated games they can complete within same second
            ...
            timestamp = int(time.time() * 1000)
            log_filename = f"{base}_{timestamp.replace(':', '.')}{ext}"
