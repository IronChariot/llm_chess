        ...
        **kwargs: Any,
        ...
        try:
            ...
            if legal_moves:
                ...
                # Do a basic check to see if the move is a string made up of alphanumeric characters (or a +) and length 2 to 5
                if not re.match(r"^[a-zA-Z0-9+#=\-]{2,7}$", random_move):
                    raise ValueError("Invalid move provided")

                # try:
                #     # Check if the move is valid in UCI format
                #     print(f"Checking if the move {random_move} is valid in UCI format")
                #     move_obj = chess.Move.from_uci(random_move)
                #     if move_obj not in self.board.legal_moves:
                #         raise ValueError("Invalid move provided")
                # except Exception:
                #     print(f"Checking if the move {random_move} is valid in SAN format")
                #     try:
                #         move_obj = self.board.parse_san(random_move)
                #     except Exception:
                #         print(f"Invalid move provided: {random_move}")
                #         raise ValueError("Invalid move provided")
                        # Throws if invalid move is provided, i.e. if previous action was not get_legal_moves
                # At this point, move_obj is a legal move. We return it in whatever format it's in (san or uci)
                self.flip_flag = True
...
class AutoReplyAgent(GameAgent):
    ...
    def generate_reply(
        ...
        **kwargs: Any,
        ...
        action_choice = messages[-1]["content"].strip()
        ...
        if self.get_current_board_action in action_choice:
            ...
            # E.g. make_move e2e4 OR make_move c2c1r OR (if it's a SAN move) make_move f3 OR make_move Kf1
            match = re.search(
                rf"{self.make_move_action} ([a-zA-Z0-9+#=\-]{{2,7}})", action_choice
            )
            if match:
                try:
                    move = match.group(1)
